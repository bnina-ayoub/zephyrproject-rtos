 ...
 */
struct mcux_eqdc_inputmux_entry {
	INPUTMUX_Type *base;
	uint16_t channel;
	uint32_t connection;
};
...
struct mcux_eqdc_config {
	...
	const struct mcux_eqdc_inputmux_entry *inputmux_entries;
			      ...
			      enum sensor_attribute attr, const struct sensor_value *val)
{
	...
	if ((enum sensor_attribute_mcux_eqdc)attr != SENSOR_ATTR_EQDC_COUNTS_PER_REVOLUTION) {
		return -ENOTSUP;
	}

	if (val->val1 < 1 || (uint32_t)val->val1 > (UINT32_MAX / 4U)) {
		LOG_ERR("Counts per revolution value invalid: %d", val->val1);
		return -EINVAL;
	}

	/* Transfer counts because it works on Quadrature X4 mode */
	data->counts_per_revolution = (uint32_t)val->val1 * 4U;
...
static DEVICE_API(sensor, mcux_eqdc_api) = {
...
static int mcux_eqdc_init_pins(const struct device *dev)
{
	const struct mcux_eqdc_config *config = dev->config;
	int ret;
	...
	for (uint8_t i = 0; i < config->inputmux_entries_count; i++) {
		const struct mcux_eqdc_inputmux_entry *entry = &config->inputmux_entries[i];
...
#define MCUX_EQDC_INPUTMUX_ENTRY(node_id, prop, idx)					\
	{										\
...
#define MCUX_EQDC_INPUTMUX_DEFINE(n)							\
	COND_CODE_1(DT_INST_NODE_HAS_PROP(n, inputmux_connections),			\
		    (static const struct mcux_eqdc_inputmux_entry			\
			    mcux_eqdc_inputmux_entries_##n[] = {			\
			    DT_INST_FOREACH_PROP_ELEM_SEP(n, inputmux_connections,	\
							  MCUX_EQDC_INPUTMUX_ENTRY, (,))}; \
		    ), ())

#define MCUX_EQDC_INPUTMUX_INIT(n)							\
	COND_CODE_1(DT_INST_NODE_HAS_PROP(n, inputmux_connections),			\
		    (.inputmux_entries = mcux_eqdc_inputmux_entries_##n,		\
...
#define MCUX_EQDC_DEVICE_INIT(inst)						\
	...
	MCUX_EQDC_IRQ_CONFIG(inst)						\
										\
	MCUX_EQDC_INPUTMUX_DEFINE(inst)						\
										\
